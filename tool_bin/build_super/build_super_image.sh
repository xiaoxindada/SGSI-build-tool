...
super_new_img="$LOCALDIR/super_new.img"
rm -rf $super_new_img
...
create_super_image() {
  ...
  local sparse_image=true # create sparse format super.img
  local base_cmd="--metadata-size 65536 --super-name super"
  [ $sparse_image = true ] && cmd_args+=" --sparse"

  # a_only ext args
  for image in ${images[@]}; do
    local partition_name=$(echo $image | sed 's/\.img//g')
    local size=$(stat -c %s $image)
    local ext_a_only_cmd="$ext_a_only_cmd --partition $partition_name:readonly:$size --image $partition_name=$image"
  done

  # ab ext args
  for partition_name in $(echo ${images[@]} | sed -e 's/\.img//g' -e 's/_a\b//' -e 's/_b\b//'); do
    local size=$(stat -c %s ${partition_name}.img)
    local ext_ab_cmd="$ext_ab_cmd --partition ${partition_name}_a:readonly:$size --image ${partition_name}_a=${partition_name}.img"
  done

  # virtual_ab args
  for partition_name in $(echo ${images[@]} | sed -e 's/\.img//g' -e 's/_a\b//' -e 's/_b\b//'); do
    local size=$(stat -c %s ${partition_name}.img)
    local ext_virtual_ab_cmd="$ext_virtual_ab_cmd --partition ${partition_name}_a:readonly:$size --image ${partition_name}_a=${partition_name}.img --partition ${partition_name}_b:readonly:0"
  done
  ...
  while true; do
    ...
    "a_only")
      ...
      final_cmd="$base_cmd --metadata-slots 2 $ext_a_only_cmd"
      break
      ;;
    "ab")
      base_cmd+=" --device super:$super_final_size"
      ...
      final_cmd="$base_cmd --metadata-slots 3 $ext_ab_cmd"
      break
      ;;
    "virtual_ab")
      base_cmd+=" --device super:$super_final_size"
      base_cmd+=" --group ${super_group}_a:$super_final_size"
      base_cmd+=" --group ${super_group}_b:$super_final_size"
      final_cmd="$base_cmd --metadata-slots 3 --virtual-ab $ext_virtual_ab_cmd"
      break
      ;;
    *)
      echo "输入错误！清重试"
